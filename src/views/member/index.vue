<template>
  <div class="h-full bg-zinc-200 dark:bg-zinc-800 duration-400 xl:pt-1">
    <div
      ...
      <div class="py-2 px-1">
        <h2 class="text-center text-2xl font-bold tracking-widest text-yellow-600">
          精选VIP
        </h2>
        ...
        <div
          ...
          <pay-menu-item-vue v-for="item in vipPayList" :key="item.id" :hot="item.isHot"
        ...
        <!-- 支付 -->
        <payment-vue class="mt-2" :payData="currentPayData" />
      </div>
    </div>
  </div>
</template>
...
const getVipListData = async () => {
  const res = await getVipPayList()
  vipPayList.value = res
